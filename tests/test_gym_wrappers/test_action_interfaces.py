...
class TestScaledActionInterface:
    ...
    interface = ScaledActionInterface(gym.make('BipedalWalker-v3'), scaled_action_range=(0, 1))
    ...
    def test_action_to_tensor_direct(self):
        action_tensor = self.interface.action_to_tensor(np.array([-1, 0.0, 0.5, 1]))
        print('action_tensor:', action_tensor, '\n')
        print('action_scaler', self.interface.scaler.inrange, self.interface.scaler.outrange)
        torch.testing.assert_allclose(action_tensor, torch.FloatTensor([0.0, 0.5, 0.75, 1.0]))

    def test_tensor_to_action_direct(self):
        action_arr = self.interface.tensor_to_action(torch.FloatTensor([0.0, 0.5, 0.75, 1.0]))
        print('action_arr:', action_arr, '\n')
        np.testing.assert_allclose(action_arr, np.array([-1, 0.0, 0.5, 1], 'float32'))
