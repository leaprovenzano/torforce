...
def get_groups(mask):
    groups = (mask * 1).flip(0).cumsum(0).flip(0)
...
def discount_case(draw, rewards):
    ...
    terminals = draw(tensors('bool', shape, elements=st.booleans()))
    return reward, terminals


class TestDiscount:

    basic_reward_strat = float_tensors(
        min_dims=1, max_dims=2, min_side=1, max_side=10, elements=st.floats(-100, 100, width=32)
    )
    ones = float_tensors(
        min_dims=1, max_dims=2, min_side=1, max_side=10, elements=st.floats(1, 1, width=32)
    )
    ...
    def test_simple_gamma_one(self, inp):
        ...
        result = discount(inp, terminals=terms, gamma=1.0)
    ...
    def test_simple_gamma_zero(self, inp):
        ...
        terms = terms != 0
        print(terms)
        result = discount(inp, terminals=terms, gamma=0.0)
        ...
        if not terms.all():
            for grp, msk in group_split(result, mask=terms, return_mask=True):
                ...
                for v in grp.flip(0).tolist():
                    pytest.approx(v, 0.00001) == expected
